...
class learnmath.mathml.edit.DisplayMathML{
	...
	public function getText():String{
		...
		for(var i=0; i<listOfXml.length; i++){
			...
			s = s + addChildren(_xmlML.childNodes, 0) +  "<br>";
		}
		return s;
	}
		...
		for(var i=0; i<children.length; i++){
			...
			if(children[i].nodeName!=null){
				if(children[i].nodeName=="mo" | children[i].nodeName=="mtext"){
					s = s + insertTabs(tabs) + "<span class='"+classTag+"'>&lt;"+ children[i].nodeName;
					s = s + insertAttributes(children[i]);
					s = s + "&gt;</span>";
					s = s + addChildren(children[i].childNodes, tabs+1);
					s = s + "<span class='"+classTag+"'>&lt;/"+ children[i].nodeName + "&gt;</span><br>";
				}else{
					s = s + insertTabs(tabs) + "<span class='"+classTag+"'>&lt;"+ children[i].nodeName;
					s = s + insertAttributes(children[i]);
					s = s + "&gt;</span><br>";
					s = s + addChildren(children[i].childNodes, tabs+1);
					s = s + insertTabs(tabs) + "<span class='"+classTag+"'>&lt;/"+ children[i].nodeName + "&gt;</span><br>";
				}
			}else{
